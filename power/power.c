...
void *thread_uevent(__attribute__((unused)) void *x)
{
    while (1) {
        ...
        if (nevents == -1) {
            if (errno == EINTR)
                continue;
            ALOGE("%s: thread_uevent: poll_wait failed\n", LOG_TAG);
            break;
        }
        ret = uevent_event();
        if (ret < 0)
            ALOGE("%s: Error processing the uevent event", LOG_TAG);
    }
    return NULL;
}
...
static void kai_power_init( __attribute__((unused)) struct power_module *module)
{
    ALOGI("%s init.\n", LOG_TAG);
    /*
